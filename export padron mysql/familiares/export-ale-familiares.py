...
def conect():
    ...
    query = "SELECT * FROM familia WHERE cuil or nrodoc !=0 group by nrodoc"
...
def main():
    ...
    for persona in a:
        ...
        query = "INSERT IGNORE INTO familiares(cuil,parentesco,nro_beneficiario,nombre,apellido,nro_doc,tipo_doc,sexo_id,fecha_nac,nacionalidad_id,verificado,mov,fecha_mov)VALUES(%i,%i,%i,\"%s\",\"%s\",%i,\'%s\',\'%s\',\'%s\',%i,%i,%i,\"%s\")"%(
            persona[12], persona[3], persona[0], persona[6], persona[7], persona[11], doc[persona[10]],
            persona[8], persona[9], nacion[persona[13]],0,1,fecha_null)
        cursor.execute(query)
        conexion.commit()
        querymov = "INSERT IGNORE INTO altasbajas(cuil,accion,fecha) values(%s,%s,\'%s\')" % (persona[12],1,fecha_null)
